...
test.describe("SEO Page", () => {
  ...
  seoTestData.forEach((data) => {
    test(
      ...
      async ({ page, seoPage }) => {
        test.setTimeout(600000); // Tăng timeout lên 10 phút để tránh lỗi timeout khi chờ PageSpeed API
        ...
        await customStep(page, "10. Xác thực Tốc độ tải trang & Core Web Vitals", async () => {
          await seoPage.verifyPerformance(vitals, localMetrics, scorecard, config.checkCoreWebVitals !== false);
        });
