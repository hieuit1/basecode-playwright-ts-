...
export class RecruitmentPage extends ArticleBasePage {
    ...
    constructor(page: Page) {
        ...
        this.clientUrl = "tuyen-dung";

        // Cấu hình riêng cho mục Tuyển dụng
        this.parentMenu = page.locator("//a[@title='Quản lý bài viết']");
        this.subMenu = page.locator("//a[@href='news?act=man&type=tuyen-dung']");
    }
    ...
    async addArticle(title: string, slug: string, desc: string, content: string, imagePath: string, enData?: any) {
        ...
        if (!enData) {
            enData = {
                titleEn: `${title} EN`,
                slugEn: `${slug}-en`,
                descEn: `English description for bulk test recruitment`,
                contentEn: `English content for bulk test recruitment`,
            };
        }
        await super.addArticle(title, slug, desc, content, imagePath, enData);
    }

    async verifyRecruitmentInAdminSuccess() {
        await this.verifyAdminSuccess();
    }

    async verifyRecruitmentOnWebsite(title: string) {
        await this.verifyOnWebsite(title);
    }

    async deleteRecruitment(title: string) {
        await this.deleteArticle(title);
    }

    // ========== Bulk Test Methods ==========
    async copyBulkRecruitments(count: number) {
        await this.copyBulkArticles(count);
    }

    async deleteBulkRecruitments(prefix: string) {
