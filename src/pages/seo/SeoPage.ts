...
export class SeoPage extends BasePage {
  ...
  async verifyMetaDescription(scan: SeoScanResult, sc: SeoScorecard) {
    ...
    await sc.check(
      `Meta description tồn tại (${metaVal !== null ? (metaVal ? metaVal.length + " ký tự" : "rỗng") : "Không tìm thấy"})`,
      metaVal !== null,
      "Thẻ <meta name=\"description\"> không tồn tại!"
    );
  }
  ...
  async verifyLinks(scan: SeoScanResult, sc: SeoScorecard) {
    ...
    // Bỏ qua các domain nội bộ công ty và MXH thường chặn bot để tránh False Positive (status 0)
    const ignoredDomains = ['mimatech.vn', 'mimadigi.com', 'facebook.com', 'zalo.me', 'tiktok.com', 'm.me', 'zalo.app'];

    const isCheckable = (href: string) => {
      if (!href || href.startsWith("#") || href.startsWith("javascript:") || href.startsWith("mailto:") || href.startsWith("tel:")) {
        return false;
      }
      return !ignoredDomains.some(domain => href.includes(domain));
    };
  ...
  async verifyPerformance(vitals: any, localMetrics: any, sc: SeoScorecard, checkCoreWebVitals: boolean = true) {
    if (!checkCoreWebVitals) {
      return;
    }
